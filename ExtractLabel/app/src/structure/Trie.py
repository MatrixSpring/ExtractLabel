...
class Trie(object):
    ...
    def __getitem__(self, prefix):
        return self.wordsWithPrefix(prefix)
    ...
    def printTree(self):
        layers = {}
        printedLayer = "layer 1: Root node\n", '      root', '=>', [self.root.get(child).getChar() for child in
                                                                    self.root.children], '\n'
    ...
    def __printTree(self, node, layer, layers):
        ...
        for child in node.getChildren():
            ...
            printedLayer = '        ', child.getChar(), '=>', [child.get(grandchild).getChar() for grandchild in
                                                               child.getChildren()], '\n'
