...
MODELDIR = "../../res/ltp/ltp_data_v3.4.0"
...
def fact_triple_extract(sentence, out_file):
    ...
    for index in range(len(postags)):
        ...
        if netags[index][0] == 'S' or netags[index][0] == 'B':
            print('netags', netags[index][0], netags[index][0], index, len(netags))
            ni = index-1
            if netags[ni][0] == 'B':
                while netags[ni][0] != 'E':
                    ni += 1
...
if __name__ == "__main__":
    ...
    out_file = 'tree-triple-extractor-3.txt'
    filePersist = FilePersist()
    list_remark = filePersist.loadLine('../../assert/foo-json.txt')

    fp = open(out_file, "w", encoding='utf-8', errors='ignore')
    for item_data in list_remark:
        substring = split_sents(item_data)
        for str in substring:
            fact_triple_extract(str, fp)
    fp.close()