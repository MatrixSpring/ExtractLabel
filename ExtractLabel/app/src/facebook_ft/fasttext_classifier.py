...
from app.src.facebook_ft import data_process, indicator
from app.src.facebook_ft.config import Config
...
class FasttextClassifier(object):
    ...
    def train(self):
        ...
        print(sys.stderr, 'Start training model, training file: %s, saved model path: %s.' % (traing_file, save_path))

        classifier = fasttext.supervised(traing_file, save_path, label_prefix='__label__', dim=100, word_ngrams=2, bucket=2000000, loss='softmax')
    ...
    def eval_model(self, model_path, eval_data_file):
        ...
        print("\nFinally, model accuracy: %.3f, precision: %.3f, recall: %.3f." % (accuracy_total / class_size, precision_total / class_size, recall_total / class_size))
